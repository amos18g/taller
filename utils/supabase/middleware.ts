...
export const updateSession = async (request: NextRequest) => {
    ...
    try {
        ...
        const isPublicPath = publicPaths.includes(requestedPath);
         ...
         if (!user && !isPublicPath) { // Si NO hay usuario y NO es ruta pública
    ...
    .single();
            ...
            if (userProfile && userProfile.role) { // Valida si el user esta logeado y tiene un rol 
                ...
                if (userRole === 'admin') {
                    ...
                    if (request.nextUrl.pathname.startsWith('/user')) { // en  caso de accesar al segmento de usuario pero es admin
                        return NextResponse.redirect(new URL('/inicio', request.url)); // redirige al area del admin
                    }
                } else { // rol usario normal
                    console.log("prueba role 2")
                    console.log(userRole)
                    if (request.nextUrl.pathname.startsWith('/admin') || request.nextUrl.pathname.startsWith('/inicio')) { // no tiene rol de admin, rutas a las que no puede acceder
